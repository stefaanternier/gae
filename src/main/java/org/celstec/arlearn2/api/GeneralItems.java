...
//		QueryGeneralItems gi = new QueryGeneralItems(token);
	...
	@DELETE
	@Path("/gameId/{gameIdentifier}/{path}")
	public String deleteFilePath(@HeaderParam("Authorization") String token,
							 @PathParam("gameIdentifier") Long gameIdentifier,
		...
		if (!validCredentials(token))
			return serialise(getInvalidCredentialsBean(), accept);
		GeneralItemDelegator gid = new GeneralItemDelegator(token);
		return serialise(gid.deleteFilePath(gameIdentifier, path), accept);
	}
...
//		UsersDelegator ud = new UsersDelegator(gid);
	...
	@POST
	@Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Path("/clone/fromGameId/{fromGameId}/toGameId/{toGameId}")
	public String cloneFilePath(@HeaderParam("Authorization") String token, String path,
								  @PathParam("fromGameId") Long fromGameId,
								  @PathParam("toGameId") Long toGameId,
								  @PathParam("pathId") Long pathId,
		...
		FilePathManager.clone(fromGameId, toGameId, split[0], split[1]);
		return "{}";
		//return
