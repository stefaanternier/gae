...
public class Actions extends Service {
    ...
    @GET
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    @Path("/runId/{runIdentifier}/user/{user}")
    public String getActionsForUser(@HeaderParam("Authorization") String token,
                                    @QueryParam("from") Long from,
                                    @QueryParam("until") Long until,
                                    @QueryParam("resumptionToken") String cursor,
                                    @PathParam("runIdentifier") Long runIdentifier,
        ...
        ActionDelegator ad = new ActionDelegator(this);
        if (from == null && until == null) {
            return serialise(ad.getActionsFromUntil(runIdentifier, user, 0l, until, cursor), accept);
        }
        return serialise(ad.getActionsFromUntil(runIdentifier, user, from, until, cursor), accept);
    }
