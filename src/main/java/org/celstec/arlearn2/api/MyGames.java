...
import org.celstec.arlearn2.upload.GameUnpacker;
...
public class MyGames extends Service {
	...
	public String getGadget(@PathParam("gameId") Long gameIdentifier, @QueryParam("height") Integer heightIn) {
		...
		Game game = gd.getGame(gameIdentifier, true);
...
//		GamesList gl = qg.getGames(cursor, from, user.getProvider(), user.getLocalId());
//
...
//				if (!gad.canView(account.getFullId(), gameIdentifier)) {
//					return serialise(getInvalidCredentialsBean(), accept);
//
//				}
//			}
...
//        }
//		return serialise(g, accept);
      return getGameWithAccessCheck(token, gameIdentifier, false, accept);
	}

	@GET
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Path("/gameId/{gameIdentifier}/checkAccess/{check}")
	public String getGameWithAccessCheck(
			@HeaderParam("Authorization") String token,
			...
			@DefaultValue("application/json") @HeaderParam("Accept") String accept)
	{
		...
		if (g.getSharing() == null || g.getSharing() == Game.PRIVATE) {
			if (!validCredentials(token)){
				...
				GameAccessDelegator gad = new GameAccessDelegator(token);
				if (check && !gad.canView( gameIdentifier)) { //account.getFullId(),
					return serialise(getInvalidCredentialsBean(), accept);
...
//            return serialise(getInvalidCredentialsBean(), accept);
	...
	@Consumes({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	@Path("/importGame")
	public String importGame(@HeaderParam("Authorization") String token, String gameString,
					  @DefaultValue("application/json") @HeaderParam("Content-Type") String contentType,
					  @DefaultValue("application/json") @HeaderParam("Accept") String accept)  {
		if (!validCredentials(token))
			return serialise(getInvalidCredentialsBean(), accept);
		...
		try {
			deserialized = JsonBeanDeserializer.deserialize(gameString);
			if (deserialized instanceof GamePackage && ((GamePackage) deserialized).getGame() != null){
				GameUnpacker gu = new GameUnpacker((GamePackage) deserialized, token);
				gu.unpack();
			}
		} catch (JSONException e) {
			e.printStackTrace();
