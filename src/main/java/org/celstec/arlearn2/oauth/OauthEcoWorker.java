...
public class OauthEcoWorker extends OauthWorker {
    ...
    static {
        OauthConfigurationEntity jdo = OauthKeyManager.getConfigurationObject(AccountEntity.ECOCLIENT);
...
//
            sendRedirect(request.getAccessToken(), ""+request.getExpires_in(), AccountEntity.ECOCLIENT);
        } else {
...
//            if (profileJson.has("picture")) picture = profileJson.getString("picture");
            ...
            AccountEntity account = AccountManager.addAccount("noFbId",id, AccountEntity.ECOCLIENT, email, given_name, family_name, name, picture, false);
