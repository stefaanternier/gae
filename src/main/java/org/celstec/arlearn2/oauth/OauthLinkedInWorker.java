...
public class OauthLinkedInWorker extends OauthWorker {
    ...
    static {
		OauthConfigurationEntity jdo = OauthKeyManager.getConfigurationObject(AccountEntity.LINKEDINCLIENT);
	...
	public void exchangeCodeForAccessToken() {
		...
		sendRedirect(request.getAccessToken(), "" + request.getExpires_in(), AccountEntity.LINKEDINCLIENT);
	...
	public void saveAccount(String accessToken) {		JSONObject profileJson;
		try {
			...
			AccountEntity account = AccountManager.addAccount("noFbId", profileJson.getString("id"), AccountEntity.LINKEDINCLIENT, profileJson.getString("emailAddress"), profileJson.getString("firstName"), profileJson.getString("lastName"), profileJson.getString("firstName") + " " + profileJson.getString("lastName"),
