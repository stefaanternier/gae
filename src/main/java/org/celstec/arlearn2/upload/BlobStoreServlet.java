...
//		System.out.println(path);
		...
		BlobKey bk = FilePathManager.getFilePathEntityByAccount(account, Long.parseLong(runIdString), fileName).get(0).getBlobKey();
		if (bk != null) {
            if (req.getParameter("thumbnail") == null) {
				BlobInfo blobInfo = bi.loadBlobInfo(bk);
...
//						BlobKey oldkey = FilePathManager.getBlobKey(account, runId, item.getName());
						BlobKey oldkey = FilePathManager.getFilePathEntityByAccount(account, runId, item.getName()).get(0).getBlobKey();
						if (oldkey != null) {
